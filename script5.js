...
window.pacoUrlPagina=function(){
 var u=(window.location.href||'').toLowerCase();
 if(u.indexOf('127.0.0.1')!==-1||u.indexOf('localhost')!==-1){return 'https://festivalpresidente.tuboleta.com.do/eventperformances.asp?evt=949';}
 if(u.indexOf('rrprc=')!==-1){
 u=u.replace('eventperformances.asp','referenciacompra.asp');}
return u;};
})();
...
/* Terminos de uso / Aviso de privacidad -> popup con iframe (checkout_streamlined.asp).
   Mismo popup que usa el footer (paco-docpopup) para "Preguntas frecuentes":
   una tarjeta modal que carga el documento por fetch+srcdoc, sin salir del
   checkout ni abrir pestana nueva. Los enlaces se configuran abajo. */
window.PACO_TERMINOS_POPUP=window.PACO_TERMINOS_POPUP||{activo:true,
 enlaces:[
  {coincide:'termsofuse.asp',url:'https://cdn.jsdelivr.net/gh/Infinitho-Devs/aquiarius@main/tc-2026.html'},
  {coincide:'privacystatement.asp',url:'https://cdn.jsdelivr.net/gh/Infinitho-Devs/aquiarius@main/politicas-2026.html'}
 ]
};
})();
;
...
(function(){if(window.PACO_EN_FRAME)return;
 ...
 return null;}
 var overlay,card,loading,errBox,errLink,iframe,closeBtn,cache={};
 function cerrar(){
 overlay.classList.remove('paco-abierto');overlay.setAttribute('aria-hidden','true');
 document.body.style.overflow='';iframe.style.display='none';iframe.removeAttribute('srcdoc');}
 function construir(){
 if(overlay)return;
 if(!document.getElementById('paco-docpopup-css')){
 var css='#paco-docpopup-overlay{position:fixed;inset:0;z-index:99999;display:none;align-items:center;justify-content:center;padding:22px;background:rgba(0,10,6,.72);}'+
 '#paco-docpopup-overlay.paco-abierto{display:flex;}'+
 '.paco-docpopup-card{position:relative;width:100%;max-width:920px;height:88vh;max-height:900px;border-radius:18px;overflow:hidden;background:#01150B;box-shadow:0 30px 70px rgba(0,0,0,.55);border:1px solid rgba(223,186,83,.25);}'+
 ...
 '@media (max-width:760px){.paco-docpopup-card{height:92vh;border-radius:14px;}}';
 var st=document.createElement('style');st.id='paco-docpopup-css';st.textContent=css;
 (document.head||document.documentElement).appendChild(st);}
 overlay=document.createElement('div');overlay.id='paco-docpopup-overlay';overlay.setAttribute('aria-hidden','true');
 overlay.innerHTML='<div class="paco-docpopup-card" role="dialog" aria-modal="true">'+
 '<button type="button" class="paco-docpopup-close" aria-label="Cerrar">&times;</button>'+
 '<div class="paco-docpopup-loading">Cargando…</div>'+
 '<div class="paco-docpopup-error" style="display:none;"><p>No se pudo cargar el documento.</p><a href="#" target="_blank" rel="noopener">Abrir en una pestaña nueva</a></div>'+
 '<iframe class="paco-docpopup-iframe" title="Documento" style="display:none;"></iframe></div>';
 document.body.appendChild(overlay);
 card=overlay.querySelector('.paco-docpopup-card');
 loading=card.querySelector('.paco-docpopup-loading');
 errBox=card.querySelector('.paco-docpopup-error');
 errLink=errBox.querySelector('a');
 iframe=card.querySelector('.paco-docpopup-iframe');
 closeBtn=card.querySelector('.paco-docpopup-close');
 ...
 function abrir(url){
 construir();
 overlay.classList.add('paco-abierto');overlay.setAttribute('aria-hidden','false');
 document.body.style.overflow='hidden';
 errBox.style.display='none';iframe.style.display='none';loading.style.display='flex';errLink.href=url;
 if(cache[url]){iframe.srcdoc=cache[url];iframe.style.display='block';loading.style.display='none';return;}
 fetch(url,{mode:'cors'}).then(function(res){if(!res.ok)throw new Error('http '+res.status);return res.text();})
 .then(function(html){cache[url]=html;iframe.srcdoc=html;iframe.style.display='block';loading.style.display='none';})
 .catch(function(){loading.style.display='none';errBox.style.display='flex';});
 }
 ...
 (function(u){a.addEventListener('click',function(e){e.preventDefault();abrir(u);});})(url);}}
 pasar();
 document.addEventListener('DOMContentLoaded',pasar);
